...
**렌더 시스템** (`src/scenes/fieldScene.js` + `src/config.js`):
...
**14개 맵 전부 elevation** (멀티 티어 + 리전별 스킴):
| 맵 | 크기 | elevation |
|---|---|---|
...
**아트**: PixelLab 환경 프롭 5종(`prop_wild_bush/mossy_boulder/crypt_rubble/ice_cluster/swamp_reeds`, `public/structures/`, `walkable:true` 장식). wild/dungeon/frost/swamp에 산포. **PixelLab 잔여 ~53 generations** (Tier2 구독).

---

## 2. 아키텍처 빠른참조 (다음 세션이 바로 손대는 노브)
...
- **라이브 QA**: dev `import.meta.env.DEV`에서 `window.__game`(scenes.top=field, .player, .map) + `window.__dbg`. 헤드리스는 Canvas2D 폴백 + 필드 절반-렌더 퀵 있음 → 실 브라우저로 시각 확인.
...
하늘·먼 산·안개 레이어를 playfield 뒤에 깔아 디오라마 깊이. **정탑다운 해법 = 맵을 화면 하단
(1-band)으로 마스킹하고 상단 band(22%)에 패럭랙스 배경을 노출**(사용자 "맵 70% + 배경" 직관).
**실내(dungeon/ice)는 `regions.<x>.off:true`로 배경 끔** → 풀스크린 맵(기존 틸트시프트 룩).
- 구현됨: `config.BACKDROP`(enabled/band/리전별 sky·hills) + `fieldScene.buildBackdrop()`(코드
  ...
  `camTarget()` 밴드 클램프(world.y ≤ band → 북단에서도 플레이어 안 가려짐) + update 패럭랙스
  (산 x = world.x×factor) + far-blur 게이트 오프.
...
- 아트: PixelLab 풀 배경 회피하고 코드 실루엣으로 감 — 충분히 읽힘(스크린샷 검증).

### B′. 촉각 생동감 (정탑다운 ROI 최상위) — ✅ SHIPPED (2026-05-31)
카메라 lerp(무게감) + 물 파문(발걸음·로머) + 풀 흔들림/갈라짐(지나가면 비켜남) + 발걸음 먼지.
정탑다운에선 원경보다 촉각이 시간당 생동감 큼(2차 의견 검증). `fieldScene` 렌더 전용, 279 그린.

### C. 환경 애니메이션 (M, 생동감)
흔들리는 풀/갈대, 물 반짝임, 떠다니는 입자. `update(dt)`에서 데코/물 타일에 sine 흔들림. 코드 중심.

### D. 계단 정렬 너그럽게 (S, 선택) — 유저 피드백 후속
계단이 1칸이라 정렬이 빡빡. 필요시 stairs를 **3칸 폭**으로 넓혀 등반 지점을 늘림. (현재 시각은 골드 램프 스팬으로 해결됨 — 실플레이 후 판단.)

### E. elevation 형태 다양화 (M)
현재 전부 사각 plateau. 가라앉은 웅덩이(주변을 올려 표현), 굽이치는 고가 통로, 비대칭 단 등. `raiseRect` 여러 번 조합 or 신규 헬퍼.

### F. 맵 크기 일관성 (M, 선택)
핵심 경로 4맵만 확대됨. empire/lava/void 맵(26×22, 18×14)도 "거리감"을 위해 확대할지 — 좌표 상대값 리팩터 필요(dungeon.js 패턴 참고: `bossX=W-3`).

### G. 무드/색 라이브 튜닝 (S)
`REGION_MOOD`/`CLIFF`/`GOLD`/`ELEV_STEP`/`WORLD_SCALE` 실 브라우저로 눈 맞추기. 특히 골드 계단 톤, 다크 밴드 농도.

---

## 4. 열린 질문 / 결정 대기
- **정탑다운 vs 오블리크 틸트**: 진짜 옥토패스 3D 디오라마 각은 정탑다운이라 구조적 불가(렌더러 재작성 필요). 현재 가짜 elevation으로 대체. 틸트 도입은 대공사 — 별도 결정.
- **"위로 누르기" vs "ledge 방향으로 누르기"**: top-down elevation은 화면-위 ≠ 고도-위. 골드 램프가 방향을 가리키지만, 신규 유저 혼란 가능 → 튜토리얼 힌트 고려.
- **PixelLab 크레딧 배분**(~53): 배경 vs 프롭 vs 절벽-strip 타일. A/B에 우선.

---

## 5. 시작점 (다음 세션 첫 액션 추천)
1. **A (프롭 산포)** 부터 — 싸고 즉각 밀도 ↑, 전 맵 일관성. empire에 crypt_rubble부터.
2. 그다음 **B (원경 배경)** — 옥토패스 격차 가장 크게 줄임.
3. 매 변경 후 `npx vitest run` (elevation/도달성 가드) + `run-qa-snapshot` 또는 실 브라우저.

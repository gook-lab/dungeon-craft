...
| 단계 | 내용 |
|---|---|
| 문제 | 전투 규칙이 렌더링 코드와 섞이면 밸런스 조정과 맵 변경이 화면을 직접 실행해야만 확인되는 구조가 됩니다. |
| 선택 | 전투 계산과 이동 규칙을 순수 로직으로 분리하고, 씬은 계산 결과를 받아 표현만 담당하게 했습니다. |
| 구현 | 시드 기반 RNG, 순수 전투 리졸버, LIFO 씬 스택, 포탈 그래프와 충돌 데이터 검사를 구성했습니다. |
| 검증 | Vitest로 전투·진행·맵 연결성을 확인하고, 헤드리스 하네스로 빌드별 평균 라운드와 사망 수를 비교합니다. |
| 회고 | 플레이 감각은 직접 확인해야 하지만, 규칙과 데이터의 오류는 렌더러 없이 반복 검증할 수 있어야 조정 속도가 빨라졌습니다. |
